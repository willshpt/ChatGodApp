...
class TTSManager:
    ...
    audio_manager = AudioManager("VB-Audio Virtual Cable")  # Use full VB-Cable device name
    obswebsockets_manager = OBSWebsocketsManager()

    user1_voice_name = "TTS"
    user1_voice_style = "random"
    user2_voice_name = "TTS"
    user2_voice_style = "random"
    user3_voice_name = "TTS"
    ...
    def text_to_audio(self, text, user_number):
        ...
        # Play the audio once (with delete_file=True to clean up)
        self.audio_manager.play_audio(tts_file, True, True, True)

        # Disable the filters after playback
        if user_number == "1":
